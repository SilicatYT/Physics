...
# (Important): This is done here so I can still use "return run" in hitbox_type/1
    ...
    execute if score #Physics.Touching Physics matches 1 run return run data modify storage physics:zprivate ContactGroups[-1].Objects[0].Blocks[-1].Hitboxes[-1].Contacts append from storage physics:temp data.NewContact
