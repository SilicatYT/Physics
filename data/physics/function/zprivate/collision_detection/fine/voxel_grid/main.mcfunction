...
# Note: Precalculation - The projection of the world-geometry voxel onto the 9 cross product axes and the axes of the object has to be done for each cube, but there's a trick: Precompute the projection of all 8 corner points of the voxel onto the axes, using only coordinates relative to the position of the cube (in this case the cube coordinate origin, not the center), then precompute the min and max for each axis. Then, for each voxel, project only the origin coordinate onto the axes and add that projection to the precomputed min and max values. If the voxel position were the coordinate origin, one of the voxel's corners would always be at [0, 0, 0], relatively speaking. So I wouldn't have to project that one onto the axes, as it would always result in [0, 0, 0]. But overall it's faster to use the center position, because then I only have to project half the corner points onto each axis, calculate the max, and then just change the sign to get the min for each axis.
