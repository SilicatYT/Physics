...
execute as @a unless score @s Physics.Player.ID matches 1.. store result score @s Physics.Player.ID run scoreboard players add #Physics Physics.Player.ID 1
...
# Line of sight: Spawn or teleport punchable interaction hitboxes
    ...
    execute as @a at @s run function physics:zprivate/line_of_sight/main

    # Kill leftover hitboxes
    # (Important): Because interaction entities can unload, players can leave etc.
    execute if score #Physics.InteractionCount Physics > #Physics.SuccessfulTeleports Physics as @e[type=minecraft:interaction,tag=Physics.Hitbox] unless score @s Physics.Hitbox.Gametime = #Physics.Gametime Physics run kill @s
