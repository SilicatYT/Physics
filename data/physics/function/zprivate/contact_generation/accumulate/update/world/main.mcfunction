...
# (Important): If the block is air, it does do some unnecessary stuff, but that's gonna be so rare that it doesn't matter.
    # Block 1
    ...
    execute store result score #Physics.BlockPos.z Physics run data get storage physics:temp data.Blocks[-1].Pos[2]
        ...
        # Check hitbox
        scoreboard players set #Physics.NoHitbox Physics 0
        execute unless data storage physics:temp data.Blocks[-1].InContact run function physics:zprivate/contact_generation/accumulate/update/world/get_hitbox/main
        ...
        execute if score #Physics.NoHitbox Physics matches 0 unless data storage physics:temp data.Blocks[-1].InContact run function physics:zprivate/contact_generation/accumulate/update/world/contacts_main
        execute if score #Physics.NoHitbox Physics matches 0 if data storage physics:temp data.Blocks[-1].InContact run function physics:zprivate/contact_generation/accumulate/update/world/contacts_main_in_contact
