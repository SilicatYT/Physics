# Append the contact
data modify storage physics:zprivate ContactGroups[-1].Objects[0].Blocks[-1].Hitboxes[-1].Contacts append value {ContactNormal:[I;-1000,0,0],ContactPoint:[I;0,0,0]}
...
execute if score #Physics.Contact.FeatureB Physics matches 10 store result storage physics:zprivate ContactGroups[-1].Objects[0].Blocks[-1].Hitboxes[-1].Contacts[-1].ContactPoint[0] int 1 run return run scoreboard players get #Physics.Projection.Block.WorldAxis.x.Min Physics
execute store success storage physics:zprivate ContactGroups[-1].Objects[0].Blocks[-1].Hitboxes[-1].Contacts[-1].ContactNormal[0] int 1000 store result storage physics:zprivate ContactGroups[-1].Objects[0].Blocks[-1].Hitboxes[-1].Contacts[-1].ContactPoint[0] int 1 run scoreboard players get #Physics.Projection.Block.WorldAxis.x.Max Physics
