...
# (Important): If the penetration depth is negative or the contact point is not inside the other object, the contact is just appended (so it can still be updated during resolution) without being updated. In that case, the separating velocity data is removed to avoid potential bugs.
    ...
    $execute if score #Physics.PenetrationDepth Physics matches ..-1 if score #Physics.Contact.FeatureA Physics matches 10 run return run function physics:zprivate/contact_generation/accumulate/world/touching/update_contact/object_axis_x/calc_penetration_stuff_negative with storage physics:temp data.BlockCorner[$(FeatureB)]
    $execute if score #Physics.PenetrationDepth Physics matches ..-1 if score #Physics.Contact.FeatureA Physics matches 11 run return run function physics:zprivate/contact_generation/accumulate/world/touching/update_contact/object_axis_x/calc_penetration_stuff_positive with storage physics:temp data.BlockCorner[$(FeatureB)]
