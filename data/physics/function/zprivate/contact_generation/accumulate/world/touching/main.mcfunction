...
# (Important): If the block has no hitbox (anymore), it does do some unnecessary stuff, but that's gonna be so rare that it doesn't matter.
    ...
    execute store result score #Physics.BlockPos.z Physics run data get storage physics:temp data.Blocks[-1].Pos[2]
        ...
        # Check hitbox
        execute unless data storage physics:temp data.Blocks[-1].InContact run function physics:zprivate/contact_generation/accumulate/world/touching/get_hitbox/main
        # AAAAAAAAAAAAAAAAAAAAAA, DONT GET THE HITBOX, IT'S ALREADY STORED DURING THE NEW CONTACT GENERATION
        ...
        execute if score #Physics.NoHitbox Physics matches 0 unless data storage physics:temp data.Blocks[-1].InContact run function physics:zprivate/contact_generation/accumulate/world/touching/contacts_main
        execute if score #Physics.NoHitbox Physics matches 0 if data storage physics:temp data.Blocks[-1].InContact run function physics:zprivate/contact_generation/accumulate/world/touching/contacts_main_in_contact
