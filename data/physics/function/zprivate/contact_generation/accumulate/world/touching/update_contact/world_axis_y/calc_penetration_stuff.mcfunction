# Append the contact
data modify storage physics:zprivate ContactGroups[-1].Objects[0].Blocks[-1].Hitboxes[-1].Contacts append value {ContactNormal:[I;0,-1000,0],ContactPoint:[I;0,0,0]}
...
$execute store result storage physics:zprivate ContactGroups[-1].Objects[0].Blocks[-1].Hitboxes[-1].Contacts[-1].ContactPoint[2] int 1 run scoreboard players get #Physics.ThisObject Physics.Object.CornerPosGlobal.$(FeatureA).z
execute if score #Physics.Contact.FeatureB Physics matches 12 store result storage physics:zprivate ContactGroups[-1].Objects[0].Blocks[-1].Hitboxes[-1].Contacts[-1].ContactPoint[1] int 1 run return run scoreboard players get #Physics.Projection.Block.WorldAxis.y.Min Physics
execute store success storage physics:zprivate ContactGroups[-1].Objects[0].Blocks[-1].Hitboxes[-1].Contacts[-1].ContactNormal[1] int 1000 store result storage physics:zprivate ContactGroups[-1].Objects[0].Blocks[-1].Hitboxes[-1].Contacts[-1].ContactPoint[1] int 1 run scoreboard players get #Physics.Projection.Block.WorldAxis.y.Max Physics
