# Get Edge B
    ...
    execute store result score #Physics.PenetrationDepth Physics run scoreboard players operation #Physics.ObjectB.EdgeProjection Physics += #Physics.Projection.BlockCenter.CrossProductAxis.yx Physics
...
# Contact Point
    ...
    # Calculate the contact point (Middle between points on edges A and B)
        # Point on EdgeA
        scoreboard players operation #Physics.Maths.s Physics *= @s Physics.Object.Axis.x.x
...
# Contact Normal
execute unless score #Physics.ObjectA.EdgeProjection Physics > #Physics.ObjectB.EdgeProjection Physics store result storage physics:zprivate ContactGroups[-1].Objects[0].Blocks[-1].Hitboxes[-1].Contacts[-1].ContactNormal[0] int 1 run scoreboard players get #Physics.CrossProductAxis.yx.x Physics
execute unless score #Physics.ObjectA.EdgeProjection Physics > #Physics.ObjectB.EdgeProjection Physics store result storage physics:zprivate ContactGroups[-1].Objects[0].Blocks[-1].Hitboxes[-1].Contacts[-1].ContactNormal[2] int 1 run return run scoreboard players get #Physics.CrossProductAxis.yx.z Physics
