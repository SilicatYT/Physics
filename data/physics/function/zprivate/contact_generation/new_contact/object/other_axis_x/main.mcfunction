...
execute if score #Physics.Projection.Object.OtherObjectAxis.x.Min Physics < @s Physics.Object.ProjectionOwnAxis.x.Min store result storage physics:temp data.NewContact.FeatureB byte 1 store result storage physics:temp data.FeatureB byte 1 run scoreboard players set #Physics.FeatureB Physics 10
execute if score #Physics.Projection.Object.OtherObjectAxis.x.Min Physics >= @s Physics.Object.ProjectionOwnAxis.x.Min store result storage physics:temp data.NewContact.FeatureB byte 1 store result storage physics:temp data.FeatureB byte 1 run scoreboard players set #Physics.FeatureB Physics 11
...
scoreboard players operation #Physics.DeepestProjection Physics = #Physics.Projection.Object.OtherObjectAxis.x.Max Physics
    ...
    execute if score #Physics.FeatureB Physics matches 10 run function physics:zprivate/contact_generation/new_contact/object/other_axis_x/check_corner_max
    execute if score #Physics.FeatureB Physics matches 11 run function physics:zprivate/contact_generation/new_contact/object/other_axis_x/check_corner_min
...
# Calculate Penetration Depth, Contact Normal, Contact Point & Separating Velocity
    ...
    execute if score #Physics.FeatureB Physics matches 10 store result storage physics:temp data.NewContact.ContactNormal[0] int -1 run scoreboard players operation #Physics.Maths.Value1 Physics = @s Physics.Object.Axis.x.x
    execute if score #Physics.FeatureB Physics matches 10 store result storage physics:temp data.NewContact.ContactNormal[1] int -1 run scoreboard players operation #Physics.Maths.Value2 Physics = @s Physics.Object.Axis.x.y
    execute if score #Physics.FeatureB Physics matches 10 store result storage physics:temp data.NewContact.ContactNormal[2] int -1 run scoreboard players operation #Physics.Maths.Value3 Physics = @s Physics.Object.Axis.x.z
    ...
    execute if score #Physics.FeatureB Physics matches 10 run scoreboard players operation #Physics.PenetrationDepth Physics *= #Physics.Constants.-1 Physics
    ...
    # (Important): To get the actual separating velocity, I subtract ObjectA's separating velocity from ObjectB's (because they face different directions, they're sign-flipped), as both objects may be moving or rotating.
        ...
        execute if score #Physics.FeatureB Physics matches 10 run scoreboard players operation #Physics.SeparatingVelocity.x Physics *= #Physics.Constants.-1 Physics
