...
scoreboard players operation #Physics.DeepestProjection Physics = #Physics.Projection.Object.CrossProductAxis.yy.Max Physics
    ...
    scoreboard players operation #Physics.DeepestProjection Physics -= #Physics.Projection.ObjectCenter.CrossProductAxis.yy Physics
    execute if score #Physics.Projection.OtherObject.CrossProductAxis.yy.Min Physics >= #Physics.Projection.Object.CrossProductAxis.yy.Min Physics run function physics:zprivate/contact_generation/new_contact/object/cross_product_axis_yy/check_edge_a_max
    execute if score #Physics.Projection.OtherObject.CrossProductAxis.yy.Min Physics < #Physics.Projection.Object.CrossProductAxis.yy.Min Physics run function physics:zprivate/contact_generation/new_contact/object/cross_product_axis_yy/check_edge_a_min

# Get ObjectB's feature (Edge that's closest to ObjectA)
# (Important): The edge is perpendicular to the axis with the minimum overlap, so it projects onto a single point. Meaning I only have to look at 1 corner point of each of the 4 candidate edges to see which one is farther along the axis.
scoreboard players operation #Physics.DeepestProjection Physics = #Physics.Projection.OtherObject.CrossProductAxis.yy.Max Physics

    # Set the feature
    # (Important): Because only the min and max projections are scaled down, I need to scale the corner projections down here and turn the DeepestProjection relative again. In addition, to account for rounding errors that are different for positive and negative values (It matters whether I first multiply by -1 and then divide, or the other way around), I turn the min back to the max and just invert which corner matches which corner projection if it tries to get the min projection's corner.
    scoreboard players operation #Physics.DeepestProjection Physics -= #Physics.Projection.OtherObjectCenter.CrossProductAxis.yy Physics
    execute if score #Physics.Projection.OtherObject.CrossProductAxis.yy.Min Physics >= #Physics.Projection.Object.CrossProductAxis.yy.Min Physics run function physics:zprivate/contact_generation/new_contact/object/cross_product_axis_yy/check_edge_b_min
    execute if score #Physics.Projection.OtherObject.CrossProductAxis.yy.Min Physics < #Physics.Projection.Object.CrossProductAxis.yy.Min Physics run function physics:zprivate/contact_generation/new_contact/object/cross_product_axis_yy/check_edge_b_max
...
execute if score #Physics.ObjectA.EdgeProjection Physics < #Physics.ObjectB.EdgeProjection Physics store result storage physics:temp data.NewContact.ContactNormal[0] int 1 store result score #Physics.ContactNormal.x Physics run scoreboard players get #Physics.CrossProductAxis.yy.x Physics
execute if score #Physics.ObjectA.EdgeProjection Physics < #Physics.ObjectB.EdgeProjection Physics store result storage physics:temp data.NewContact.ContactNormal[1] int 1 store result score #Physics.ContactNormal.y Physics run scoreboard players get #Physics.CrossProductAxis.yy.y Physics
execute if score #Physics.ObjectA.EdgeProjection Physics < #Physics.ObjectB.EdgeProjection Physics store result storage physics:temp data.NewContact.ContactNormal[2] int 1 store result score #Physics.ContactNormal.z Physics run scoreboard players get #Physics.CrossProductAxis.yy.z Physics
...
# Calculate Penetration Depth, Contact Point & Separating Velocity
    ...
    # (Important): Edge1 = <Corner on the negative end> + s * <Corner on the positive end> => (u + s * v)    //    Edge2 = <Corner on the negative end> + t * <Corner on the positive end> => (m + t * n)
        # Calculate intermediate results (Dot products)
            ...
            scoreboard players operation #Physics.Maths.C Physics *= @s Physics.Object.Axis.y.x
            ...
            scoreboard players operation #Physics.Maths.Value6 Physics *= @s Physics.Object.Axis.y.y
            ...
            scoreboard players operation #Physics.Maths.Value6 Physics *= @s Physics.Object.Axis.y.z
            scoreboard players operation #Physics.Maths.C Physics += #Physics.Maths.Value6 Physics
        ...
        scoreboard players operation #Physics.ContactPoint.x Physics = #Physics.ThisObject Physics.Object.Axis.y.x
    ...
    # (Important): I calculate t and the closest point on ObjectB's edge to ObjectA's edge, then compute the distance between both points.
        ...
        # Calculate the distance (Point calculation with t is interweaved for performance reasons)
            ...
            scoreboard players operation #Physics.Maths.Value1 Physics *= @s Physics.Object.Axis.y.x
            scoreboard players operation #Physics.Offset.y Physics *= @s Physics.Object.Axis.y.y
            scoreboard players operation #Physics.Offset.z Physics *= @s Physics.Object.Axis.y.z
            scoreboard players operation #Physics.Maths.Value1 Physics /= #Physics.Constants.1000 Physics
            ...
            execute store result storage physics:temp data.NewContact.PenetrationDepth short 1 store result storage physics:zprivate ContactGroups[-1].Objects[-1].MaxPenetrationDepth short 1 run scoreboard players get #Physics.Maths.SquareRoot.Output Physics
...
execute if score #Physics.Maths.SquareRoot.Output Physics > @s Physics.Object.MaxPenetrationDepth if score @s Physics.Object.MaxPenetrationDepth = @s Physics.Object.MaxPenetrationDepth.World store result storage physics:zprivate ContactGroups[-1].Objects[-1].Contacts[-1].HasMaxPenetrationDepth byte 0 store result storage physics:zprivate ContactGroups[-1].MaxPenetrationDepthIsObjectContact byte 0 run data remove storage physics:zprivate ContactGroups[-1].Objects[0].Blocks[].Hitboxes[].Contacts[{HasMaxPenetrationDepth:0b}].HasMaxPenetrationDepth
execute if score #Physics.Maths.SquareRoot.Output Physics > @s Physics.Object.MaxPenetrationDepth unless score @s Physics.Object.MaxPenetrationDepth = @s Physics.Object.MaxPenetrationDepth.World store result storage physics:zprivate ContactGroups[-1].Objects[-1].Contacts[-1].HasMaxPenetrationDepth byte 0 run data remove storage physics:zprivate ContactGroups[-1].Objects[].Contacts[{HasMaxPenetrationDepth:0b}].HasMaxPenetrationDepth
