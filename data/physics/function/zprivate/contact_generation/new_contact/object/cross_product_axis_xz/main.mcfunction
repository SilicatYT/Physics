...
scoreboard players operation #Physics.DeepestProjection Physics = #Physics.Projection.Object.CrossProductAxis2.xz.Max Physics
    ...
    scoreboard players operation #Physics.DeepestProjection Physics -= #Physics.Projection.ObjectCenter.CrossProductAxis2.xz Physics
    execute if score #Physics.Projection.OtherObject.CrossProductAxis2.xz.Min Physics >= #Physics.Projection.Object.CrossProductAxis2.xz.Min Physics run function physics:zprivate/contact_generation/new_contact/object/cross_product_axis_xz/check_edge_a_max
    execute if score #Physics.Projection.OtherObject.CrossProductAxis2.xz.Min Physics < #Physics.Projection.Object.CrossProductAxis2.xz.Min Physics run function physics:zprivate/contact_generation/new_contact/object/cross_product_axis_xz/check_edge_a_min

# Get ObjectB's feature (Edge that's closest to ObjectA)
# (Important): The edge is perpendicular to the axis with the minimum overlap, so it projects onto a single point. Meaning I only have to look at 1 corner point of each of the 4 candidate edges to see which one is farther along the axis.
scoreboard players operation #Physics.DeepestProjection Physics = #Physics.Projection.OtherObject.CrossProductAxis2.xz.Max Physics

    # Set the feature
    # (Important): Because only the min and max projections are scaled down, I need to scale the corner projections down here and turn the DeepestProjection relative again. In addition, to account for rounding errors that are different for positive and negative values (It matters whether I first multiply by -1 and then divide, or the other way around), I turn the min back to the max and just invert which corner matches which corner projection if it tries to get the min projection's corner.
    scoreboard players operation #Physics.DeepestProjection Physics -= #Physics.Projection.OtherObjectCenter.CrossProductAxis2.xz Physics
    execute if score #Physics.Projection.OtherObject.CrossProductAxis2.xz.Min Physics >= #Physics.Projection.Object.CrossProductAxis2.xz.Min Physics run function physics:zprivate/contact_generation/new_contact/object/cross_product_axis_xz/check_edge_b_min
    execute if score #Physics.Projection.OtherObject.CrossProductAxis2.xz.Min Physics < #Physics.Projection.Object.CrossProductAxis2.xz.Min Physics run function physics:zprivate/contact_generation/new_contact/object/cross_product_axis_xz/check_edge_b_max
...
execute if score #Physics.ObjectA.EdgeProjection Physics < #Physics.ObjectB.EdgeProjection Physics store result storage physics:temp data.NewContact.ContactNormal[0] int 1 store result score #Physics.ContactNormal.x Physics run scoreboard players get #Physics.CrossProductAxis2.xz.x Physics
execute if score #Physics.ObjectA.EdgeProjection Physics < #Physics.ObjectB.EdgeProjection Physics store result storage physics:temp data.NewContact.ContactNormal[1] int 1 store result score #Physics.ContactNormal.y Physics run scoreboard players get #Physics.CrossProductAxis2.xz.y Physics
execute if score #Physics.ObjectA.EdgeProjection Physics < #Physics.ObjectB.EdgeProjection Physics store result storage physics:temp data.NewContact.ContactNormal[2] int 1 store result score #Physics.ContactNormal.z Physics run scoreboard players get #Physics.CrossProductAxis2.xz.z Physics
