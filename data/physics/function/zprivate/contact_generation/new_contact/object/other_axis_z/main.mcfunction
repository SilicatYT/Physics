...
execute if score #Physics.Projection.Object.OtherObjectAxis.z.Min Physics < @s Physics.Object.ProjectionOwnAxis.z.Min store result storage physics:temp data.NewContact.FeatureB byte 1 store result storage physics:temp data.FeatureB byte 1 run scoreboard players set #Physics.FeatureB Physics 14
execute if score #Physics.Projection.Object.OtherObjectAxis.z.Min Physics >= @s Physics.Object.ProjectionOwnAxis.z.Min store result storage physics:temp data.NewContact.FeatureB byte 1 store result storage physics:temp data.FeatureB byte 1 run scoreboard players set #Physics.FeatureB Physics 15
...
scoreboard players operation #Physics.DeepestProjection Physics = #Physics.Projection.Object.OtherObjectAxis.z.Max Physics
    ...
    execute if score #Physics.FeatureB Physics matches 14 run function physics:zprivate/contact_generation/new_contact/object/other_axis_z/check_corner_max
    execute if score #Physics.FeatureB Physics matches 15 run function physics:zprivate/contact_generation/new_contact/object/other_axis_z/check_corner_min
...
# Calculate Penetration Depth, Contact Normal, Contact Point & Separating Velocity
    ...
    execute if score #Physics.FeatureB Physics matches 14 store result storage physics:temp data.NewContact.ContactNormal[0] int -1 run scoreboard players operation #Physics.Maths.Value1 Physics = @s Physics.Object.Axis.z.x
    execute if score #Physics.FeatureB Physics matches 14 store result storage physics:temp data.NewContact.ContactNormal[1] int -1 run scoreboard players operation #Physics.Maths.Value2 Physics = @s Physics.Object.Axis.z.y
    execute if score #Physics.FeatureB Physics matches 14 store result storage physics:temp data.NewContact.ContactNormal[2] int -1 run scoreboard players operation #Physics.Maths.Value3 Physics = @s Physics.Object.Axis.z.z
    ...
    execute if score #Physics.FeatureB Physics matches 14 run scoreboard players operation #Physics.PenetrationDepth Physics *= #Physics.Constants.-1 Physics
    ...
    # (Important): To get the actual separating velocity, I subtract ObjectA's separating velocity from ObjectB's (because they face different directions, they're sign-flipped), as both objects may be moving or rotating.
        ...
        # Separating Velocity for ObjectA
            ...
            # Calculate the relative velocity's dot product with the contact normal to get the separation velocity (single number, not a vector) and store it
            # (Important): Because the contact normal scores (Physics.Object.Axis.?.?) are not adjusted to face away from the face, I may multiply the final result with -1.
            scoreboard players operation #Physics.SeparatingVelocity.x Physics *= @s Physics.Object.Axis.z.x
            scoreboard players operation #Physics.SeparatingVelocity.y Physics *= @s Physics.Object.Axis.z.y
            scoreboard players operation #Physics.SeparatingVelocity.z Physics *= @s Physics.Object.Axis.z.z
        ...
        execute if score #Physics.FeatureB Physics matches 14 run scoreboard players operation #Physics.SeparatingVelocity.x Physics *= #Physics.Constants.-1 Physics
