...
# Calculate Penetration Depth, Contact Normal, Contact Point & Separating Velocity
    ...
    # (Important): Edge1 = <Corner on the negative end> + s * <Corner on the positive end> => (u + s * v)    //    Edge2 = <Corner on the negative end> + t * <Corner on the positive end> => (m + t * n)
        ...
        # Calculate the contact point (Middle between points on edges A and B)
            ...
            scoreboard players operation #Physics.OtherPoint.y Physics *= @s Physics.Object.Axis.z.y
            ...
            scoreboard players operation #Physics.OtherPoint.z Physics *= @s Physics.Object.Axis.z.z
    ...
    execute unless score #Physics.ObjectA.EdgeProjection Physics > #Physics.ObjectB.EdgeProjection Physics store result storage physics:temp data.NewContact.ContactNormal[1] int 1 run scoreboard players operation #Physics.ContactNormal.y Physics = #Physics.CrossProductAxis.yz.y Physics
    execute unless score #Physics.ObjectA.EdgeProjection Physics > #Physics.ObjectB.EdgeProjection Physics store result storage physics:temp data.NewContact.ContactNormal[2] int 1 run scoreboard players operation #Physics.ContactNormal.z Physics = #Physics.CrossProductAxis.yz.z Physics
