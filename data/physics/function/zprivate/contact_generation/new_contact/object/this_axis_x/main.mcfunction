...
execute if score #Physics.Projection.OtherObject.ObjectAxis.x.Min Physics < #Physics.ThisObject Physics.Object.ProjectionOwnAxis.x.Min store result storage physics:temp data.NewContact.FeatureA byte 1 store result storage physics:temp data.FeatureA byte 1 run scoreboard players set #Physics.FeatureA Physics 10
execute if score #Physics.Projection.OtherObject.ObjectAxis.x.Min Physics >= #Physics.ThisObject Physics.Object.ProjectionOwnAxis.x.Min store result storage physics:temp data.NewContact.FeatureA byte 1 store result storage physics:temp data.FeatureA byte 1 run scoreboard players set #Physics.FeatureA Physics 11
...
scoreboard players operation #Physics.DeepestProjection Physics = #Physics.Projection.OtherObject.ObjectAxis.x.Max Physics
    ...
    execute if score #Physics.FeatureA Physics matches 10 run function physics:zprivate/contact_generation/new_contact/object/this_axis_x/check_corner_max
    execute if score #Physics.FeatureA Physics matches 11 run function physics:zprivate/contact_generation/new_contact/object/this_axis_x/check_corner_min
...
# Calculate Penetration Depth, Contact Normal, Contact Point & Separating Velocity
    ...
    execute if score #Physics.FeatureA Physics matches 10 store result storage physics:temp data.NewContact.ContactNormal[0] int -1 run scoreboard players operation #Physics.Maths.Value1 Physics = #Physics.ThisObject Physics.Object.Axis.x.x
    execute if score #Physics.FeatureA Physics matches 10 store result storage physics:temp data.NewContact.ContactNormal[1] int -1 run scoreboard players operation #Physics.Maths.Value2 Physics = #Physics.ThisObject Physics.Object.Axis.x.y
    execute if score #Physics.FeatureA Physics matches 10 store result storage physics:temp data.NewContact.ContactNormal[2] int -1 run scoreboard players operation #Physics.Maths.Value3 Physics = #Physics.ThisObject Physics.Object.Axis.x.z
    ...
    # (Important): To get the actual separating velocity, I subtract ObjectA's separating velocity from ObjectB's (because they face different directions, they're sign-flipped), as both objects may be moving or rotating.
        ...
        execute if score #Physics.FeatureA Physics matches 10 run scoreboard players operation #Physics.SeparatingVelocity.x Physics *= #Physics.Constants.-1 Physics
