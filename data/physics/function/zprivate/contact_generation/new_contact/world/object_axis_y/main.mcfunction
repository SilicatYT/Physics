...
execute if score #Physics.Projection.Block.ObjectAxis.y.Min Physics < @s Physics.Object.ProjectionOwnAxis.y.Min store result storage physics:temp data.NewContact.FeatureA int 1 store result storage physics:temp data.FeatureA int 1 run scoreboard players set #Physics.ObjectA.Feature Physics 102
execute if score #Physics.Projection.Block.ObjectAxis.y.Min Physics >= @s Physics.Object.ProjectionOwnAxis.y.Min store result storage physics:temp data.NewContact.FeatureA int 1 store result storage physics:temp data.FeatureA int 1 run scoreboard players set #Physics.ObjectA.Feature Physics 103
...
# Calculate Penetration Depth, Contact Normal, Contact Point & Separating Velocity
    ...
    execute if score #Physics.ObjectA.Feature Physics matches 102 store result storage physics:temp data.NewContact.ContactNormal[0] int -1 run scoreboard players operation #Physics.Maths.Value1 Physics = @s Physics.Object.Axis.y.x
    execute if score #Physics.ObjectA.Feature Physics matches 102 store result storage physics:temp data.NewContact.ContactNormal[1] int -1 run scoreboard players operation #Physics.Maths.Value2 Physics = @s Physics.Object.Axis.y.y
    execute if score #Physics.ObjectA.Feature Physics matches 102 store result storage physics:temp data.NewContact.ContactNormal[2] int -1 run scoreboard players operation #Physics.Maths.Value3 Physics = @s Physics.Object.Axis.y.z
    ...
    # (Important): The separating velocity is the dot product between the contact point's relative velocity and the contact normal. The relative velocity is the cross product between the angular velocity and the contact point (relative to the object's center) that's added together with the object's linear velocity.
        ...
        execute if score #Physics.ObjectA.Feature Physics matches 102 run scoreboard players operation #Physics.PointVelocity.x Physics *= #Physics.Constants.-1 Physics
