...
# Calculate Penetration Depth, Contact Normal, Contact Point & Separating Velocity
    ...
    # (Important): The separating velocity is the dot product between the contact point's relative velocity and the contact normal. The relative velocity is the cross product between the angular velocity and the contact point (relative to the object's center) that's added together with the object's linear velocity.
        ...
        # (Important): I mixed up ObjectA and ObjectB. With two objects, you take one object's pointVelocity and subtract it from the other to get the relativeVelocity (contactVelocity). The one that's subtracted is the object whose feature is the face, so I flip the sign if FeatureA is 11 instead of 10.
        ...
        execute if score #Physics.FeatureA Physics matches 11 run scoreboard players operation #Physics.PointVelocity.x Physics *= #Physics.Constants.-1 Physics
