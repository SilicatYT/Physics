...
# Calculate Penetration Depth, Contact Normal, Contact Point & Separating Velocity
    ...
    # (Important): Because the static object is now the one with the face, the result of the separating velocity is inverted.
        ...
        execute if score #Physics.ObjectB.Feature Physics matches 104 run scoreboard players operation #Physics.ContactPoint.x Physics *= #Physics.Constants.-1 Physics
        execute store result storage physics:temp data.NewContact.SeparatingVelocity int 1 run scoreboard players get #Physics.ContactPoint.x Physics
...
execute if score #Physics.ContactPoint.x Physics < #Physics.MinSeparatingVelocityTotal Physics store result storage physics:zprivate data.ContactGroups[-1].MinSeparatingVelocity int 1 run return run scoreboard players operation #Physics.MinSeparatingVelocityTotal Physics = #Physics.ContactPoint.x Physics
execute store result storage physics:zprivate data.ContactGroups[-1].MinSeparatingVelocity int 1 run scoreboard players get #Physics.ContactPoint.x Physics
