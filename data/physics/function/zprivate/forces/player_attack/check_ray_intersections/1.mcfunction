...
execute store result score #Physics.Maths.RayIntersection.t1 Physics.Value run scoreboard players operation #Physics.Maths.Temp.Value1 Physics.Value /= #Physics.Maths.RayDirectionLocal.x Physics.Value

# Step 2: If t is negative, stop. This means no collision with this face is happening.
execute if score #Physics.Maths.Temp.Value1 Physics.Value matches ..-1 run return run scoreboard players set #Physics.Maths.RayIntersection.t1 Physics.Value 2147483647
...
# Step 3: Calculate the point of intersection for the other two axes: origin + t * direction
    ...
    execute unless score #Physics.Maths.Temp.Value1 Physics.Value matches -500000..500000 run return run scoreboard players set #Physics.Maths.RayIntersection.t1 Physics.Value 2147483647

    # z
    scoreboard players operation #Physics.Maths.Temp.Value1 Physics.Value = #Physics.Maths.RayIntersection.t1 Physics.Value
    ...
    execute unless score #Physics.Maths.Temp.Value1 Physics.Value matches -500000..500000 run scoreboard players set #Physics.Maths.RayIntersection.t1 Physics.Value 2147483647
