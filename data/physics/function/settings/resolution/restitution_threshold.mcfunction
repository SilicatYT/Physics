...
execute if score #Physics.Value Physics matches ..-1 run tellraw @s [{text:"Physics >> ",color:"#99EAD6"},{text:"The restitution threshold must not be below -500 (-0.5 blocks/tick).",color:"red"}]
execute if score #Physics.Value Physics matches ..-1 at @s run return run playsound minecraft:entity.villager.no master @s ~ ~ ~
execute if score #Physics.Value Physics matches 251.. run tellraw @s [{text:"Physics >> ",color:"#99EAD6"},{text:"The restitution threshold must not exceed 0 (0.0 blocks/tick).",color:"red"}]
